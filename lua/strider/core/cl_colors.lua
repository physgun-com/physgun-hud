...
strider.colors = strider.colors or {}

function strider.colors.Lerp(amt, from, to)
    return Color(
...
function strider.colors.IsLight(col)
    return (col.r * 0.299 + col.g * 0.587 + col.b * 0.114) > 186
end

function strider.colors.Rainbow()
    return HSVToColor(CurTime(), 0.9, 0.9)
end

function strider.colors.FromHex(hex)
    local str = hex
    if #str == 3 then
